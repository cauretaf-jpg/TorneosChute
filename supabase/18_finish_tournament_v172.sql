-- Chute Plataforma 1.7.2 · Finalización robusta de torneos
-- Ejecutar después de supabase/17_fix_finish_tournament_1_7_1.sql.
-- Crea una función nueva para cerrar torneos sin depender de la firma/cache de la función anterior.
...
create policy tournament_summaries_select_related_v172
on public.tournament_summaries
for select
to authenticated
using (public.can_access_tournament(tournament_id));

create policy tournament_summaries_write_creator_v172
on public.tournament_summaries
for all
to authenticated
...
create or replace function public.finish_chute_tournament_v172(
...
declare
  ...
  v_champion_user_id uuid := p_champion_user_id;
  v_champion_team_id text := p_champion_team_id;
...
begin
  ...
  -- Si el cliente no envía campeón, se infiere desde la final de playoff o desde la tabla.
  if v_champion_user_id is null and v_tournament.format = 'knockout' then
    select
      coalesce(
        winner_user_id,
        penalty_winner_user_id,
        case
          when home_goals > away_goals then home_user_id
          when away_goals > home_goals then away_user_id
          else null
        end
      ),
      case
        when coalesce(winner_user_id, penalty_winner_user_id, case when home_goals > away_goals then home_user_id when away_goals > home_goals then away_user_id else null end) = home_user_id then away_user_id
        when coalesce(winner_user_id, penalty_winner_user_id, case when home_goals > away_goals then home_user_id when away_goals > home_goals then away_user_id else null end) = away_user_id then home_user_id
        else null
      end
    into v_champion_user_id, v_runner_up_user_id
    from public.matches
    where tournament_id = p_tournament_id
      and (stage = 'final' or round = 'Final')
      and home_user_id is not null
      and away_user_id is not null
      and home_goals is not null
      and away_goals is not null
  ...
  if v_champion_user_id is null then
    raise exception 'No se pudo calcular el campeón.';
  end if;
  ...
  if v_champion_team_id is null then
    if v_tournament.team_selection_mode = 'fixed' then
      ...
      select case when home_user_id = v_champion_user_id then home_team_id when away_user_id = v_champion_user_id then away_team_id else null end
      into v_champion_team_id
      from public.matches
      where tournament_id = p_tournament_id
        and (home_user_id = v_champion_user_id or away_user_id = v_champion_user_id)
        and home_goals is not null
        and away_goals is not null
        and coalesce(result_status, 'confirmed') = 'confirmed'
      order by case when stage = 'final' or round = 'Final' then 0 else 1 end, sort_order desc nulls last, played_at desc nulls last, created_at desc
      limit 1;
    end if;
  end if;

  if v_runner_up_user_id is null then
    with played as (
      select *
      from public.matches
      where tournament_id = p_tournament_id
      ...
      select
        ...
        sum(case when gf > gc then 1 else 0 end)::int as pg
      from sides
      where user_id is not null and user_id <> v_champion_user_id
      group by user_id
    )
    select user_id into v_runner_up_user_id
    from standings
    order by pts desc, dg desc, gf desc, pg desc, user_id asc
    limit 1;
  end if;

  if v_runner_up_user_id is not null then
    if v_tournament.team_selection_mode = 'fixed' then
      ...
      select case when home_user_id = v_runner_up_user_id then home_team_id when away_user_id = v_runner_up_user_id then away_team_id else null end
      into v_runner_up_team_id
      from public.matches
      where tournament_id = p_tournament_id
        and (home_user_id = v_runner_up_user_id or away_user_id = v_runner_up_user_id)
        and home_goals is not null
        and away_goals is not null
        and coalesce(result_status, 'confirmed') = 'confirmed'
      order by case when stage = 'final' or round = 'Final' then 0 else 1 end, sort_order desc nulls last, played_at desc nulls last, created_at desc
  ...
  with played as (
    ...
    from team_sides where team_id is not null group by team_id
  )
  ...
  with goals as (
    ...
    from public.match_goal_events
    where tournament_id = p_tournament_id and nullif(trim(assist_name), '') is not null
    group by assist_name, team_id
  ), players as (
    select coalesce(g.player_name, a.player_name) as player_name,
  ...
  select player_name, team_id, goals, assists, goals + assists
  into v_best_player_name, v_best_player_team_id, v_best_player_goals, v_best_player_assists, v_best_player_contributions
  from players
  order by (goals + assists) desc, goals desc, assists desc, player_name asc
  limit 1;
  ...
  values (
    ...
    coalesce(v_played_matches, 0),
    coalesce(v_total_goals, 0),
    now(),
    jsonb_build_object(
      'source', 'finish_chute_tournament_v172',
      'playedMatches', coalesce(v_played_matches, 0),
      'totalGoals', coalesce(v_total_goals, 0)
    ),
    now()
  )
  ...
  v_summary := jsonb_build_object(
    ...
    'championUserId', v_champion_user_id,
    'championTeamId', v_champion_team_id,
    'playedMatches', coalesce(v_played_matches, 0),
    'totalGoals', coalesce(v_total_goals, 0)
  );
...
grant execute on function public.finish_chute_tournament_v172(uuid, uuid, text) to authenticated;

-- Mantiene compatibilidad con versiones previas de la app.
-- PostgreSQL no permite cambiar el tipo de retorno de una función existente con CREATE OR REPLACE.
-- Por eso primero se elimina la firma anterior, si existe.
...
as $$
begin
  return public.finish_chute_tournament_v172(p_tournament_id, p_champion_user_id, p_champion_team_id);
end;
$$;
